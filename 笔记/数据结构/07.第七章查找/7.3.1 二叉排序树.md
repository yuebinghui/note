...
### 当删除的结点有左、右两棵子树 10:00

#### 用Z结点的***直接后继***来替代被删除的结点Z，然后***直接后继***再用前两种删除的情况中的一种来恢复下二叉树
...
#### 用Z结点的***直接前驱***来替代被删除的结点Z，然后***直接前驱***再用前两种删除的情况中的一种来恢复下二叉树
...
### 时间复杂度，平均查找长度 $\lceil log_2(n+1) \rceil $
...
（大概不是说从50数到粉色框框是对比了4次，而是粉色框框和50比1次，再和26比2次，再和21比三次，所以第四次对比失败的情况其实是比了3次，*同样*第5层对比失败是对比了4次）

这里的3代表的是对比关键字的次数，7代表的时候有七种对比失败的情况
3*7就是 对比关键字次数为3的有7种对比情况，所以3\*7就有21次对比失败的情况

最后除以9, 9 是查找失败的概率是1/9

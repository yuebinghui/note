...
###### 五叉查找树：最少1个关键字，2个分叉；最多4个关键字，5个分叉
...
###### 这个地方我们虽然画出了这样的一个失败节点。但是其实对于失败结点的这些指针来说，其实就是指问了一个none
...
###### m叉查找树中,规定除了根节点外,任何结点至少有[m/2]个分叉,即至少含有[m/2]-1个关键字
...
###### 在b数当中，我们通常会习惯把这些失败节点称为***叶子节点***

###### 而最下面这一层含有实际数据的这些节点，我们会把它称为***终端节点***

###### *B树中所被允许的孩子个数的最大值称为B树的阶，通常用m表示，如下图右下角的那个结点有五个分支（五颗子树），那么就是五阶*

![image-20231031190332520](/Users/yuebinghui/Documents/program/github/note/images/image-20231031190332520.png)

## B树性质

### 1. 树中每个结点至多有m棵子树（即m个分叉），即至多含有m-1个关键字
...
###### 最小高度：让每个结点尽可能的满，有m-1个关键字，m个分叉 ***$h >= log_m(n+1)$*** 

![image-20231031195626402](/Users/yuebinghui/Documents/program/github/note/images/image-20231031195626402.png)

### 最大高度（叶子结点的下限角度入手） 21:35 🤯 

###### n个关键字的B树必有n+1个叶子结点

###### 最大高度—让各层的分叉尽可能的少，即根节点只有2个分叉，其他结点只有m/2个分叉 ***h<= $log_{\left\lceil m/2 \right\rceil}\frac{n+1}{2} + 1$*** 

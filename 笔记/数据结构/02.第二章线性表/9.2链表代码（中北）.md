...
void sort(LinkList &L) {
    ...
    // 如果只有一个结点的链表的话，必然有序，大于1个结点的链表的话，就让p后移一位
    p = r;
    ...
    while (p != NULL) {
        ...
        // 看有序链表是否有下一个结点，
        // 如果有下个结点， pre是已经有序的链表里的结点 p是当前还未排序链表中所指的结点
      // 如果pre->next的结点值小于 p结点，就让pre后移一位，因为要让p结点插入到pre结点的后面，因为我把你大嘛
      
      // 如果pre->next的结点值大于等于 p结点 那么就不需要让pre后移了，因为我当前的pre就比你大了，你就不需要插入到我后面了

...
###### 双链表代码、初始化、找到逻辑序号位置的结点、删除p的后继结点、删除、销毁线性表、判空、求长度、输出线性表、 在p结点后插入s节点、插入、头插法、尾插法
...
//    return true;
    ...
    while (pre->next != NULL) {
        p = pre->next;
        if (p->data > e) {
            DeleteNextDNode(pre);  // 删除当前循环指针p所指的结点
        } else {
            pre = p;            // 仅当未删除时移动pre
        }
    }

...
计算公式：对于M行N列矩阵b[i][j]（行列号从0开始）
$LOC + (i*N+j)$

° LOC+（ixn+j×L。 计算原理：先计算目标元素前面有多少个完整行（i n），再加上当前行的列偏移量（j）

<img src="/Users/yuebinghui/Documents/program/github/note/images/image-20230919140202571.png" alt="image-20230919140202571" style="zoom:25%;" />
